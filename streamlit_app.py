...
3.  **Copy** the generated HTML code at the bottom.
""",
...
3.  **Salin** kode HTML yang dihasilkan di bagian bawah.
""",
...
def generate_html_code():
    ...
    # Start of the table tag with styles
    html = [
        '<!-- Generated by Streamlit Table Maker -->',
    ...
    for c in range(col_count):
        ...
        html.append(f'      <th style="border: 1px solid {border_color}; text-align: left; padding: 12px; color: {text_color};">{header_text}</th>')
    html.append('    </tr>')
    html.append('  </thead>')
    ...
    for r in range(1, row_count):
        ...
        for c in range(col_count):
            ...
            html.append(f'      <td style="border: 1px solid {border_color}; text-align: left; padding: 8px; color: {text_color};">{cell_text}</td>')
        html.append('    </tr>')
    html.append('  </tbody>')
